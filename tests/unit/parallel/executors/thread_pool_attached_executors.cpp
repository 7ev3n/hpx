...
int hpx_main(int argc, char* argv[])
{
    ...
    {
        hpx::parallel::static_queue_attached_executor exec(0, num_threads);
        test_thread_pool_attached_executor(exec);
    }
#endif

    {
        hpx::parallel::local_priority_queue_attached_executor exec(0, num_threads);
        test_thread_pool_attached_executor(exec);
    }

#if defined(HPX_HAVE_STATIC_PRIORITY_SCHEDULER)
    {
        hpx::parallel::static_priority_queue_attached_executor exec(0, num_threads);
        test_thread_pool_attached_executor(exec);
    }
#endif
